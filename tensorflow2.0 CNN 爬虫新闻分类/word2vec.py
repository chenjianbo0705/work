...
def embedding_sentences(sentences, embedding_size=128, window=5, min_count=5, file_to_load='./model1.model', file_to_save=None):
...
def test():
    ...
    x_train,y_train,x_text, y_test = read_file.load_positive_negative_data_files(keji_data_file, nba2_data_file,nba_data_file)
    sentences, max_document_length = read_file.padding_sentences(x_text, '<PADDING>')
    vectors = embedding_sentences(sentences, embedding_size=32, min_count=1)
